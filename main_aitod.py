...
def get_args_parser():
    ...
    parser.add_argument('--visualize', action='store_true', default=True,
                        help='save GT and predicted boxes for all test images')
    parser.add_argument('--vis_score_thresh', type=float, default=0.5,
                        help='minimum prediction score to include in visualizations')
    parser.add_argument('--vis_output_dir', type=str, default='output/sea_vis',
    ...
    parser.add_argument('--is_vis', default=False,)


    parser.add_argument('--save_best_only', default=False,
                        help='only save best checkpoint according to validation metric')
    parser.add_argument('--best_metric_index', default=0, type=int,
                        help='metric index in coco_eval_bbox: 0=mAP, 1=AP50, 2=AP75')
    parser.add_argument('--best_ckpt_name', default='best.pth',
                        help='best checkpoint filename')
...
def main(args):
    ...
    # args.output_dir = os.path.join(args.output_dir, args.dataset_file)
    ...
    args.resume = resolve_resume_path(args.resume, args.output_dir)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        map_regular = test_stats['coco_eval_bbox'][args.best_metric_index]
        _isbest = best_map_holder.update(map_regular, epoch, is_ema=False)

        if args.output_dir and args.save_best_only and _isbest:
            weights = {
                'model': model_without_ddp.state_dict(),
                'optimizer': optimizer.state_dict(),
                'lr_scheduler': lr_scheduler.state_dict(),
                'epoch': epoch,
                'args': args,
                'best_metric': map_regular,
            }
            if args.use_ema:
                weights.update({
                    'ema_model': ema_m.module.state_dict(),
                })
            utils.save_on_master(weights, output_dir / args.best_ckpt_name)
