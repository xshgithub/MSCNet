...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, masks, pos_embeds, dn_targets, args_dn):
        ...
        # print("counting_output mean/std:", counting_output.mean().item(), counting_output.std().item())
        # print("cgfe_out mean/std:", cgfe_out.mean().item(), cgfe_out.std().item())
        # memory = cgfe_out
        memory = memory + cgfe_out
        _, predicted = torch.max(counting_output.data, 1)
        if self.dynamic_query_list:
