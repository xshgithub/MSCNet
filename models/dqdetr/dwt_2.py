...
class DWT(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # outputs = [x + H_f * x for x in inputs]
        return outputs
...
class DSFusion(nn.Module):
    ...
    def forward(self, inputs: List):
        ...
        for i in range(len(inputs)):
            inputs[i] += stack_gate * self.convs[i](inputs[i])
...
class MultiBackbone(nn.Module):
    def __init__(self, backbones, channels_per_layer, return_dwt_feats=False):
        ...
        self.Fusion = nn.ModuleList([
            ...
            # VSSFusion(channels_per_layer[0][i] * Bottleneck.expansion * len(self.backbones)) for i in range(self.num_layers)
        ])
...
def build_dwt_backbone(num_classes=2, modalities=['sla', 'ssh', 'sst', 'chl']):
    ...
    # backbone_dict = {'sla': sla, 'ssh': ssh, 'sst': sst, 'chl': chl}
    # for mod in modalities:
    #     backbone_dict[mod] =
    # multi_backbone = [backbone_dict[mod] for mod in modalities]
    multi_backbone = [
        ResNet(Bottleneck, [3, 4, 6, 3], num_classes=num_classes)
        for _ in modalities
    ]
    channels_per_layer = [[64, 128, 256, 512] for _ in modalities]
    model = MultiBackbone(multi_backbone, channels_per_layer)
