...
class HungarianMatcher(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        num_classes = pos_cost_class.shape[1]

        if tgt_ids.numel() > 0:
            label_min = tgt_ids.min().item()
            label_max = tgt_ids.max().item()

            if label_min < 0 or label_max >= num_classes:
                raise ValueError(
                    f"Target label越界："
                    f"unique labels={torch.unique(tgt_ids).cpu().tolist()}, "
                    f"label range=[{label_min}, {label_max}], "
                    f"predicted num_classes={num_classes}, "
                    f"pred_logits shape={outputs['pred_logits'].shape}"
                )
