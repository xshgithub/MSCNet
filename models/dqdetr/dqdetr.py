...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, log=True):
        ...
        scale = 2.0
        loss_ce = sigmoid_focal_loss(src_logits * scale, target_classes_onehot, num_boxes,
        ...
        # print(
        #     "target:",
        #     torch.bincount(target_classes_o, minlength=self.num_classes).tolist(),
        #     "pred:",
        #     torch.bincount(pred, minlength=self.num_classes).tolist(),
        # )
        return losses
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes):
        ...
        def safe_giou_loss(src_boxes, target_boxes, eps=1e-6):
            src_boxes = src_boxes.clamp(min=eps, max=1.0)
            target_boxes = target_boxes.clamp(min=eps, max=1.0)
            ...
            def sanitize_xyxy(boxes):
                ...
                x2_ = torch.max(x2_, x1_ + eps)
                y2_ = torch.max(y2_, y1_ + eps)
                return torch.stack([x1_, y1_, x2_, y2_], dim=-1)
            src_xyxy = sanitize_xyxy(src_xyxy)
            tgt_xyxy = sanitize_xyxy(tgt_xyxy)
            giou = box_ops.generalized_box_iou(src_xyxy, tgt_xyxy)
            giou = torch.nan_to_num(giou, nan=0.0, posinf=0.0, neginf=0.0)
            loss_giou = 1.0 - torch.diag(giou)
            return loss_giou
        loss_giou = safe_giou_loss(src_boxes, target_boxes)
    ...
    def get_loss(self, loss, outputs, targets, indices, num_boxes, **kwargs):
        loss_map = {
            ...
            'duplicate': self.loss_duplicate,  # ✅ 新增
            'area': self.loss_area,
        }
        assert loss in loss_map, f'do you really want to compute {loss} loss?'
        return loss_map[loss](outputs, targets, indices, num_boxes, **kwargs)

    def loss_duplicate(self, outputs, targets, indices, num_boxes):
        assert 'pred_boxes' in outputs

        idx = self._get_src_permutation_idx(indices)
        boxes = outputs['pred_boxes'][idx]  # 只取 matched box

        if boxes.shape[0] <= 1:
            return {'loss_duplicate': torch.tensor(0., device=boxes.device)}

        dist = torch.cdist(boxes, boxes, p=2)

        dist = torch.clamp(dist, min=1e-6)

        loss = torch.exp(-dist).mean()

        return {'loss_duplicate': loss / num_boxes}

    def loss_area(self, outputs, targets, indices, num_boxes):
        assert 'pred_boxes' in outputs
        ...
        return {'loss_area': loss / num_boxes}
    ...
    def forward(self, outputs, targets, return_indices=False):
        ...
        if torch.isnan(outputs_without_aux["pred_boxes"]).any():
            print("⚠️ NaN detected in outputs_without_aux['pred_boxes']")
            print(outputs_without_aux["pred_boxes"][0, :5])
            exit()
        indices = self.matcher(outputs_without_aux, targets)

        for i, t in enumerate(targets):
            if len(t["labels"]) == 0:
                print(f"⚠️ Target {i} has 0 labels!")

        if torch.isnan(outputs["pred_logits"]).any() or torch.isnan(outputs["pred_boxes"]).any():
            print("⚠️ NaN detected in model outputs")
...
class PostProcess(nn.Module):
    ...
    def forward(self, outputs, target_sizes, target_num=300, not_to_xyxy=False, test=False):
        ...
        # print(cls.sigmoid().mean().item(), cls.softmax(-1).mean().item())
...
def build_dqdetr(args):
    ...
    ###  新增 loss  ###
    weight_dict['loss_duplicate'] = args.loss_duplicate_coef
    weight_dict['loss_area'] = args.loss_area_coef
    weight_dict['cardinality_error'] = args.cardinality_loss_coef
    ...
    losses = ['labels', 'boxes', 'cardinality', 'duplicate', 'area']
    if args.masks:
        losses += ["masks"]
